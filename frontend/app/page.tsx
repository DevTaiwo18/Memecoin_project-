...
export default function Dashboard() {
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'Buy Now' | 'Keep Watching' | 'Likely Rug'>('all');
  ...
  const filtered = coins
    .filter(c => activeFilter === 'all' || c.score?.signal === activeFilter)
    .filter(c => {
    ...
    <div className="min-h-screen bg-[#080810] text-white">
      ...
      <div className="border-b border-white/5 px-4 md:px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-500 hidden sm:inline">Live {lastUpdated}</span>
        </div>
      </div>
      ...
      <div className="max-w-350 mx-auto px-4 md:px-8 py-6 md:py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8">
          ...
          <button onClick={() => setActiveFilter(activeFilter === 'Likely Rug' ? 'all' : 'Likely Rug')} className={`cursor-pointer text-left rounded-2xl px-4 md:px-5 py-4 border transition-all ${activeFilter === 'Likely Rug' ? 'bg-red-500/20 border-red-500/50' : 'bg-red-500/5 border-red-500/20 hover:bg-red-500/10'}`}>
            <div className="text-xl md:text-2xl font-bold text-red-400">{coins.filter(c => c.score?.signal === 'Likely Rug').length}</div>
